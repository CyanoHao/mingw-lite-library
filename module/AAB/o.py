...
def opencv(ver: BranchProfile, paths: ProjectPaths, config: argparse.Namespace):
  ...
  with overlayfs_ro('/usr/local', [
    ...
    # Fix pkgconfig missing webp's private dependency
    pkgconfig_path = paths.layer.opencv / 'lib/pkgconfig/opencv4.pc'
    with open(pkgconfig_path, 'a') as f:
      f.write('Requires.private: libwebp\n')
